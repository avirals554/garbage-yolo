...
def train(model_size: str, epochs: int, batch: int, imgsz: int, resume: bool, device: str):
...
def validate(weights_path: str):
    """Evaluate fine-tuned model on the validation set and print mAP metrics."""
    model = YOLO(weights_path)
    results = model.val(data=str(DATASET_YAML), imgsz=DEFAULT_IMGSZ)
    print("\n=== Validation Results ===")
...
def main():
    ...
    if args.validate:
        ...
        train(args.model, args.epochs, args.batch, args.imgsz, args.resume, args.device)
